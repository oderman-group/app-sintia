...
// Validar que la factura pertenezca al usuario actual (estudiante)
$consultaFactura = mysqli_query($conexion, "SELECT fcu_id, fcu_usuario FROM ".BD_FINANCIERA.".finanzas_cuentas 
    WHERE fcu_id = '".mysqli_real_escape_string($conexion, $idFactura)."' 
    AND fcu_usuario = '".$_SESSION["id"]."' 
    AND institucion = {$config['conf_id_institucion']} 
    AND year = {$_SESSION["bd"]}");

if (mysqli_num_rows($consultaFactura) == 0) {
    echo json_encode([
        'success' => false,
