...
class Estudiantes {
    ...
    public static function listarEstudiantesParaDocentesMT(array $datosCargaActual = [])
    {
        ...
        try {
            $sql="SELECT * FROM ".$baseDatosServicios.".mediatecnica_matriculas_cursos
            INNER JOIN ".BD_ACADEMICA.".academico_matriculas mat ON mat.mat_eliminado=0 AND (mat.mat_estado_matricula=1 OR mat.mat_estado_matricula=2) AND mat.mat_id=matcur_id_matricula AND mat.institucion={$config['conf_id_institucion']} AND mat.year={$_SESSION["bd"]}
            ...
            WHERE matcur_id_curso='".$datosCargaActual['car_curso']."' AND matcur_id_grupo='".$datosCargaActual['car_grupo']."' AND matcur_estado='".ACTIVO."' AND matcur_years={$_SESSION["bd"]}  AND matcur_id_institucion='".$config['conf_id_institucion']."'
            ORDER BY mat.mat_primer_apellido, mat.mat_segundo_apellido, mat.mat_nombres;
            ";
    ...
    public static function contarEstudiantesParaDocentesMT(array $datosCargaActual = [])
    {
        ...
        try {
            ...
            LEFT JOIN ".BD_GENERAL.".usuarios uss ON uss_id=mat.mat_id_usuario AND uss.institucion={$config['conf_id_institucion']} AND uss.year={$_SESSION["bd"]}
            LEFT JOIN ".$baseDatosServicios.".opciones_generales ON ogen_id=mat.mat_genero
            WHERE matcur_id_curso='".$datosCargaActual['car_curso']."' AND matcur_id_grupo='".$datosCargaActual['car_grupo']."' AND matcur_years={$_SESSION["bd"]} AND matcur_estado='".ACTIVO."' AND matcur_id_institucion='".$config['conf_id_institucion']."'
            ORDER BY mat.mat_primer_apellido, mat.mat_segundo_apellido, mat.mat_nombres;
            ");
